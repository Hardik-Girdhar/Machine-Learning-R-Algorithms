...
# Reshape the metrics for plotting
dt_metrics_long <- gather(dt_metrics, key = "variable", value = "value")

# Plotting Decision Tree model's evaluation metrics
ggplot(dt_metrics_long, aes(x = variable, y = value, fill = variable)) +
  geom_bar(stat = "identity") +
  theme_minimal() +
  ggtitle("Decision Tree Model Evaluation Metrics") +
  ylab("Metric Value") +
  xlab("Metrics") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
